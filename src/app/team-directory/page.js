...
const ROLE_OPTIONS = ["Super Admin", "Admin", "Developer", "Viewer"];
...
export default function TeamDirectoryPage() {
  ...
  useEffect(() => {
    ...
    // Basic visit log; replace with analytics/event tracking if needed
    console.log("Visited Team-directory page");
  }, []);
  ...
  const handleDelete = async (u) => {
    setOpenKebabId(null);
    const name = u.fullName || u.email || "this member";
    if (!window.confirm(`Remove ${name} from the team? This will deactivate their account.`)) return;
    try {
      setError(null);
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editUser.id == null) {
        // Create new user via backend invite endpoint (creates SQL row, random password, sends email)
        ...
        const updatePayload = {
          name: fullName,
          email: editUser.email || "",
          is_active: editUser.active !== false,
        };
        if (canManage) updatePayload.role = editUser.role || "Viewer";
                      ...
                      <div className="rowActionWrap" ref={isOpen ? kebabRef : null}>
                        ...
                        {isOpen && (
                          <div className="rowActionDropdown">
                            ...
                            {canManage && (
                              <button type="button" className="rowActionItem rowActionItemDanger" onClick={() => handleDelete(u)}>
                                Delete
                              </button>
                            )}
                          </div>
        ...
        <div className="modalBackdrop" onClick={() => setEditUser(null)}>
          <div className="modalCard editUserModal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="editUserForm">
              <div className="editUserRow">
                ...
                <div className="editUserFields">
                  ...
                  <div className="formField formFieldFull">
                    <label className="formLabel">Role access</label>
                    {canManage ? (
                      <select
                      ...
                      <div className="formInput" style={{ opacity: 0.85, cursor: "default" }} aria-readonly>
            ...
            <div className="modalFooter">
              ...
              <button type="button" className="primaryBtn primaryBtnBlue" onClick={handleSave} disabled={saving}>
                {saving ? "Saving..." : "Save"}
              </button>
            </div>
          </div>
