...
import { DateRangeFilterDropdown } from "../components/DatePickerCalendar";
import "../css/dashboard.css";
import "../css/UploadRFP.css";
...
export default function UploadRFP() {
  ...
  const {
    ...
    queryKey: ["upload-rfp", "list", user?.id, activeTab, rfpPage],
    queryFn: () =>
      rfpQuestionsApi.list({
        skip: (rfpPage - 1) * RFP_PAGE_SIZE,
        limit: RFP_PAGE_SIZE,
        user_id: user.id,
        status: tabToStatus[activeTab] ?? undefined,
      }),
    enabled: !!user?.id,
    staleTime: 2 * 60 * 1000,
  });
  ...
  const totalDocs = listData?.total ?? 0;
  const [listErrorOverride, setListErrorOverride] = useState("");
  const listError = listErrorOverride || listErrorObj?.message || "";

  const invalidateRfpList = () => {
    setListErrorOverride("");
    queryClient.invalidateQueries({ queryKey: ["upload-rfp", "list"] });
  };
  ...
  return (
    <>
      ...
      <p className="docPageHint" style={{ marginBottom: 16, color: "#666", fontSize: 14 }}>
        ...
        <Link href="/file-repository" style={{ color: "var(--primaryPurple, #6366f1)", fontWeight: 500 }}>File Repository</Link>.
      </p>
      ...
      <div className="docToolbar">
        <div className="docToolbarLeft">
          ...
          />
          {/* Reusable date range picker (2 calendars + presets); styles from DatePickerCalendar.css */}
                            ...
                            <button
                              ...
                              onClick={async (e) => {
                                ...
                                try {
                                  ...
                                  setListErrorOverride(err?.message || "Delete failed");
                                }
                              }}
                            >
                              Delete
